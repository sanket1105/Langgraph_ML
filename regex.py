...
def relocate_imports_inside_function(code_text):
    ...
    This function now properly handles conditional imports and preserves the code structure.
    ...
    """
    # Match all import statements (including conditional ones)
    import_pattern = r"^\s*(import\s+[^\n]+|from\s+\S+\s+import\s+[^\n]+)\s*$"
    imports = re.findall(import_pattern, code_text, re.MULTILINE)

    # Remove standalone imports from the top-level code
    code_without_imports = re.sub(
        import_pattern, "", code_text, flags=re.MULTILINE
    ).strip()

    # Find the function definition and insert the imports inside it
    # Fixed regex to handle multi-line function definitions
    ...
    if match:
        ...
        for imp in imports:
            imp = imp.strip()
            # Check if this import should be conditional
            if any(lib in imp.lower() for lib in ["optuna", "mlflow"]):
        ...
        if standard_imports:
            import_section.extend([f"    {imp}" for imp in standard_imports])
            import_section.append("")  # Add blank line

        # Add conditional imports
        if conditional_imports:
            # Check if there are optuna imports
            optuna_imports = [
                imp for imp in conditional_imports if "optuna" in imp.lower()
            ]
            if optuna_imports:
                import_section.append("    # Optional imports")
                import_section.append("    if enable_optuna:")
                for imp in optuna_imports:
                    import_section.append(f"        {imp}")
                import_section.append("")

            # Check if there are mlflow imports
            mlflow_imports = [
                imp for imp in conditional_imports if "mlflow" in imp.lower()
            ]
            if mlflow_imports:
                import_section.append("    if enable_mlflow:")
                for imp in mlflow_imports:
                    import_section.append(f"        {imp}")
                import_section.append("")
...
def format_recommended_steps(
    raw_text: str, heading: str = "# Recommended Steps:"
) -> str:
